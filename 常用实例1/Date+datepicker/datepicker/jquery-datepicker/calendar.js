(function() {
  ...
  DateRangePicker = (function() {
      ...
      $('body').click(function(evt) {
        if (evt.target === self.$select[0] && self.isHidden) {
          ...
          return self.show();
        	...
        	//当框消失的时候开始检索表格
        	var activeId = $(".content_left .db1.active").data("id");
        	//console.log(activeId == null)
        	//弹窗不存在的情况下开始检索
        	if($("#modal_add_user").is(":hidden")){
        		activeId == null || activeId == "" || activeId == undefined ? Plan.handleRecords(activeId,true) : Plan.handleRecords(activeId) 
        	}
    ...
    DateRangePicker.prototype.showCustomDate = function() {
      ...
      this.$select.find('option:last-child').val(text);
