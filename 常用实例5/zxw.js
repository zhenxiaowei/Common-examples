...
var zxw = function () {
    ...
    //获取UID拼接为对象  var obj = zxw.queryURLParmeter(window.location.search);

    var queryURLParmeter = function (url) {
    ...
    //倒计时
    var getSpanTime = function (string) {
   ...
   //myform为form的id
    var formPrompt = function (myform) {
    ...
    //jQuery方法post、get、put、dele、jsonp
    var Ajax = {
        //jQuery方法zxw.post(url,data,function(msg){},function(msg){alert(msg.msg);})
        post: function (url, data, succCallback, failcallback) {
            $.ajax({
                type: "post",
                url: url,
                data: data,
        ...
        //jQuery方法zxw.get(url,data,function(msg){},function(msg){alert(msg.msg);})
        get: function (url, succcallback, failcallback, data) {
            $.ajax({
                type: "get",
                url: url,
                data: data || "",
        ...
        // jQuery方法zxw.put(url,data,function(msg){},function(msg){alert(msg.msg);})
        put: function (url, data, succCallback, failcallback) {
            $.ajax({
                type: "put",
                url: url,
                data: data,
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    try {
                        if (XMLHttpRequest.responseJSON) {
                            var data = XMLHttpRequest.responseJSON || "";
                            if (data.msg == "没有登录" || data.msg == "没有权限") {
                                window.location.href = webroot + "/login.jsp";
                            } else {
                                failcallback(data);
                            }
                        } else {
        ...
        // jQuery方法zxw.dele(url,function(msg){},function(msg){alert(msg.msg);})
        dele: function (url, succcallback, failcallback, data) {
            $.ajax({
                type: "delete",
                url: url,
                data: data || "",
...
//});
    ...
    return {
        reg: reg,  //常用正则验证表达式
        queryURLParmeter: queryURLParmeter,//获取UID拼接为对象
        getContextPath: getContextPath,//获取根路径
        getTime: getTime,// 获取时间
        getDates: getDates,//转换时间戳
        getSpanTime: getSpanTime,//倒计时
        Arabia_To_SimplifiedChinese: Arabia_To_SimplifiedChinese,//阿拉伯数字转换为简写汉字
        Arrays: Arrays,// 取数组最小值、最大值、对象合并、数组求和、数组排序从小到大、数组排序从大到小、数组去重
        sum: sum,  //任意数求和
        getCode: getCode,   //随机验证码
        rnd: rnd,    //rnd:兼容版的求一定范围的随机数
        formPrompt: formPrompt, //用户登录验证规则
        string: string, //字符串方法 字符串分割为数组、字符串长度截取(从某一个字符向前截取)、字符串全部替换、两个字符串拼接（合并）
        isElement: isElement,//是否是字符串、数字、对象、数组、时间、boolean、函数、null、undefined
        emptyFun: emptyFun,//判断是否为空
        isPC: isPC,   //是否为PC端
        noteTitle: noteTitle, //自定义notetitle方法   (需要增加jQuery文件)
        Ajax: Ajax, //jQuery方法post、get、put、dele、jsonp
        goTop: goTop,   //返回顶部
        stopBubble: stopBubble,   //阻止冒泡（兼容IE）
        clone: clone,   //深度拷贝数组或对象
    }
}();
