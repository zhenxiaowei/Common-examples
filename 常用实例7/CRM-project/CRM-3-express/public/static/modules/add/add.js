...
if (obj["id"]) {
    isFlag = true;
    get("/add/getData?id=" + obj["id"], function (data) {
        console.log(data[0]);
...
$("#submit").off("click").on("click", function (e) {
    ...
    if (isFlag) {//->当前属于修改
        resObj["id"] = obj["id"];
        post("/add/updateCustom", JSON.stringify(resObj), function (data) {
    ...
    //->当前属于增加
    post("/add/addCustom", JSON.stringify(resObj), function (data) {
        showSuccess(data.message);
        setTimeout(function () {
            window.location.href = "/";
        }, 1000);
