    ...
    if (pathname === "/getAllData") {
        ...
        req.on("end", function () {
            try {
                ...
                var pageNo = temp.pageNo;
                var pageSise = temp.pageSize;
                resObj = {};
                resObj.code = allData.length > 0 ? 0 : 1;

                var count = (pageNo - 1) * pageSise;
                var ary = [];
                for (var i = count; i < allData.length; i++) {
                    ary.push(allData[i]);
                    if (ary.length == pageSise) {
                        break;
                    }
                }
                resObj.pageNo = pageNo;
                resObj.pageSise = pageSise;
                var mLength = allData.length;
                var mFirst = parseInt(mLength / pageSise);
                var b;
                if (mLength % pageSise != 0) {
                    b = mFirst + 1;
                } else {
                    b = mFirst;
                }
                resObj.totalPage = b;
                resObj.totalCount = allData.length;
    ...
    if (pathname === "/addCustom") {
        try {
            ...
            req.on("data", function (chunk) {
                //->正在获取请求主体中的内容,获取的内容是一点点获取到的,chunk这个变量中存储的是每一次获取的那一丢丢,我们在外面定义一个空字符串,每一次获取的那一丢丢我们都进行字符串拼接,这样的话就可以把全部的数据都获取到了
                temp += chunk;
                //->此处操作是异步请求处理的
            });
